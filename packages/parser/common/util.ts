 ...
 * @returns
 */
function flatPath(relativePath: string, basePath: string) {
  ...
  if (requiredFile.startsWith(PathPrefix.NODE_MODULES) || requiredFile.startsWith(PathPrefix.SRC)) {
    ...
    if (curPath.startsWith(PathPrefix.SRC))
      ...
      targetModule = findInMap(
        fs.modules,
        (item) => belongToPackage(requiredModule, item.packageName) && item.isRoot
      );
    else {
      ...
      const dependancyModule =
        module.dependancyModules.find((i) => belongToPackage(requiredModule, i.packageName)) ||
