...
function isCommonjs(text) {
  const commonjs = text
    .split('\n')
    .some(
      (line) => line.includes("require('") || line.includes('require("') || line.includes('exports')
    );
  return commonjs;
}
 ...
 * @returns
 */
async function tunnelTask<Type>(msgType: MsgType, msgData: any, event: FetchEvent) {
...
globalSelf.addEventListener('fetch', (event: FetchEvent) => {
  ...
  if (
    !url.pathname.includes(PathPrefix.NODE_MODULES) &&
