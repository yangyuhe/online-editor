...
server.on('stream', async (stream, headers) => {
  ...
  if (pathname.startsWith('/api/file?')) {
    ...
    if (!filename.startsWith('/')) filename = '/node_modules/' + filename;
    ...
    if (/(\.(tsx?|jsx?|css))/.test(path.extname(filename))) {
      ...
      async function checkPackage() {
        const package = JSON.parse(
          await fs.promises.readFile(filename + '/package.json', { encoding: 'utf8' })
        );
        let main = package.browser || package.module || package.main;
        if (package.browser && typeof package.browser === 'string') main = package.browser;
        if (main) {
      ...
      async function checkIndex() {
        ...
        const indexFile = dirs.find(
          (item) =>
            item.startsWith('index.') &&
            /\.(t|j)sx?$/.test(path.extname(item)) &&
            !item.endsWith('.d.ts')
        );
        if (indexFile) {
      ...
      async function checkSelf() {
        ...
        const selfFile = dirs.find((item) =>
          new RegExp('^' + fileName + '\\.(j|t)sx?$').test(item)
        );
        if (selfFile) {
  ...
  try {
    ...
    if (stats.isDirectory()) {
      ...
      if (ext === '.html' && content.includes('<%= importmap %>')) {
        const dirs = await fs.promises.readdir(path.resolve(__dirname, '../node_modules'), {
          encoding: 'utf8'
        });
        const importmaps = {};
        dirs.forEach((dir) => {
